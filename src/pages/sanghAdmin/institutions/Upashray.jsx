...
export default function Upashray() {
  ...
  const [activeTab, setActiveTab] = useState('Upashray');
  ...
  const stats = useMemo(() => [
    ...
    { title: 'Active Upashrays', value: upashrays.filter(p => p.status === 'Active').length, icon: Check, color: 'teal' },
    { title: 'Inactive Upashrays', value: upashrays.filter(p => p.status === 'Inactive').length, icon: X, color: 'rose' },
  ], [upashrays]);
