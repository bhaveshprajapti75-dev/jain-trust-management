...
/* Portal-based Dropdown Menu - renders outside any overflow:hidden parent */
const UploadMenuPortal = ({ onGallery, onCamera, anchorRef, onClose }) => {
  ...
  return ReactDOM.createPortal(
    <div
      style={style}
      className="bg-white rounded-2xl shadow-2xl border border-slate-100 p-2 min-w-[200px] animate-in fade-in zoom-in-95 duration-200"
    >
      <button
        type="button"
...
/* Inline Dropdown for the empty state (no overflow issues there) */
const UploadMenuInline = ({ onGallery, onCamera, positionClass }) => (
  <div className={`absolute z-[9999] bg-white rounded-2xl shadow-2xl border border-slate-100 p-2 min-w-[200px] animate-in fade-in zoom-in-95 duration-200 ${positionClass}`}>
    <button
      type="button"
      className="w-full flex items-center gap-3 p-2.5 rounded-xl hover:bg-emerald-50/50 group transition-all"
      onClick={onGallery}
...
const ImageGalleryUpload = ({
  ...
  const menuRef = useRef(null);       // wraps the trigger area for outside-click detection
  const addBtnRef = useRef(null);     // the "+" button used as portal anchor
  ...
  return (
    <div className="w-full space-y-3">
      ...
      {images.length === 0 ? (
        /* ── Empty State ── */
        <div className={`relative ${showMenu ? 'z-50' : ''}`} ref={menuRef}>
          <button
            type="button"
            disabled={disabled}
          ...
          {/* Inline dropdown – safe here, no overflow:hidden parent */}
          {showMenu && (
            <UploadMenuInline
              onGallery={() => { galleryRef.current.click(); setShowMenu(false); }}
              onCamera={() => { cameraRef.current.click(); setShowMenu(false); }}
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-4 gap-2">
            ...
            {/* '+' Upload Button – portal dropdown escapes overflow:hidden */}
            {!disabled && images.length < maxImages && (
              ...
              >
                ...
                {/* Portal dropdown – renders at document.body so overflow:hidden can't clip it */}
                {showMenu && (
                  <UploadMenuPortal
                    anchorRef={addBtnRef}
